...
public class TresNRaya {
	...
	// Cuando mover estï¿½ a true significa que la pieza seleccionada es para mover
	// Si estï¿½ a false la casilla seleccionada no est
	 ...
	 * cero seignifica que la casilla estï¿½ libre
	 * 
	 * @param coordenada
	 *            de fila
	 * @param coordenada
	...
	public boolean realizarJugada() {
		// Turno sirve para conocer a quien le correponde el turno de jugada y asï¿½
		// colocar
		// la ficha correcta
	...
	public String muestraLetrero() {
		...
		if (numerojugada <= 6)
			return "Seleccione una casilla libre";
		else {
			if (mover)
				return "Seleccione una casilla a mover";
			else
				return "Seleccione la casilla donde colocar";
		}
	...
	public String indicarAnomalia() {
		if (numerojugada <= 6) {
			if (!mirarCasillaLibre()) {
				return "La casilla no se encuentra libre";
			}
		} else {
			if (mover) {
				if (!comprobarPropiedad()) {
					return "La casilla elegida no le corresponde";
				}
				if (!comprobarBloqueada()) {
					return "La pieza seleccionada se encuentra bloqueada";
				}
			} else {
				if (!mirarCasillaLibre()) {
					return "La casilla tiene dueño";
				}
				if (!casillaContigua()) {
					return "Esta casilla no es contigua";
				}
			}
		}
		return "";
